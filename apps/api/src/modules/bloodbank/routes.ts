  ...
  app.post(
    ...
    async (request) => {
      ...
      const result = await db.$transaction(async (tx) => {
        ...
        // Sequential per-facility counter (no shared global count — avoids code
        // collisions between concurrent donations and recycling after deletes).
        const units = [];
        for (let i = 0; i < unitsCreated; i++) {
          const year = new Date().getFullYear();
          const last = await tx.bloodUnit.findFirst({ where: { facilityId: u.facilityId!, unitCode: { startsWith: `BL-${year}-` } }, orderBy: { unitCode: 'desc' }, select: { unitCode: true } });
          const nextSeq = last ? Number(last.unitCode.split('-').pop() ?? '0') + 1 : 1;
          const unit = await tx.bloodUnit.create({
            data: {
              donationId: donation.id,
              facilityId: u.facilityId!,
              unitCode: `BL-${year}-${String(nextSeq + i).padStart(4, '0')}`,
              bloodGroup,
