...
export function registerBedRoutes(app: FastifyInstance, db: PrismaClient, guards: Guards): void {
  ...
  app.get(
    ...
    async (request) => {
      ...
      return { items: beds, wards, summary };
    },
  );
  ...
  app.post(
    ...
    async (request) => {
      ...
      const admission = active ?? await db.admission.create({
        data: { patientId, facilityId: u.facilityId ?? undefined, ward: bed.ward, bed: bed.bedNumber, reason: optStr(body.reason), status: 'ADMITTED' },
      });
      const updated = await db.bed.update({
        where: { id: bed.id },
