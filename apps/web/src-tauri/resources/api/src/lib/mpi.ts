...
export interface MpiInput {
  ...
  // Foreign nationals identify by passport — scanned like a Ghana Card so a
  // duplicate foreigner is caught on the same basis as a citizen.
...
export function scoreCandidates(existing: Patient[], input: MpiInput): MpiCandidate[] {
  ...
  const passport = norm(input.passport);
  ...
  for (const p of existing) {
    ...
    const pPassport = norm(p.passport);
    ...
    if (passport && pPassport && passport === pPassport) matchedOn.push('passport');
    if (sex && p.sex && sex === p.sex.toUpperCase()) matchedOn.push('sex');

    if (matchedOn.includes('ghana_card') || matchedOn.includes('nhis') || matchedOn.includes('passport')) score = 100;
