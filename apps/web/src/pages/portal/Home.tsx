    ...
    <div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14">
        <div className="mb-6 flex items-end justify-between">
          <div>
            <h2 className="text-2xl font-bold text-g-ink">What are you looking for?</h2>
            <p className="mt-1 text-sm text-slate-500">Browse the national facility registry by type.</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
          {QUICK_FIND.map((f) => (
            <Link key={f.label} to={`/find-healthcare?type=${f.type}`} className="card-hover group rounded-xl border border-slate-200 bg-white p-5 text-center shadow-sm">
              ...
              <p className="text-xs font-semibold text-g-ink">{f.label}</p>
            </Link>
          ))}
        </div>
      </section>
      ...
      {/* Regions */}
      <section className="bg-g-mist/60 py-14">
        <div className="mx-auto max-w-7xl px-4">
          <h2 className="text-2xl font-bold text-g-ink">Find care by region</h2>
          <p className="mt-1 text-sm text-slate-500">All 16 administrative regions with their districts and facilities.</p>
          <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
            {regions.map((r) => (
              <Link key={r.id} to={`/find-healthcare?regionId=${r.id}`} className="card-hover flex items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3.5 shadow-sm">
                <div>
      ...
      {/* Features */}
      <section className="mx-auto max-w-7xl px-4 py-14">
        <h2 className="text-center text-2xl font-bold text-g-ink">One platform, built for Ghana</h2>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {[
            ...
            <div key={f.title} className="card-hover rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <h3 className="font-bold text-g-ink">{f.title}</h3>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-4">
        <div className="flex items-start gap-3 rounded-xl border border-g-red/20 bg-g-red/5 p-4">
          ...
          <div>
            <p className="text-sm font-bold text-g-ink">Public health alerts (DEMO)</p>
