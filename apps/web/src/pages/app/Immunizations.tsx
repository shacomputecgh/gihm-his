...
type Tab = 'due' | 'missed' | 'registry' | 'schedule';
...
export default function Immunizations() {
  ...
  async function remind(id: string) {
    setBusy(`remind-${id}`);
    try {
      const res = await api<ReminderResult>(`/immunizations/${id}/remind`, { method: 'POST', body: { channel: 'SMS' } });
      toast(res.dispatched ? `Reminder sent to ${res.to}` : `Reminder logged for ${res.to ?? 'no phone on file'} — dispatch pending`, 'success');
    } catch (err) {
      toast(err instanceof Error ? err.message : 'Failed', 'error');
    } finally {
      setBusy(null);
